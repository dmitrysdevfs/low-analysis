...
export async function getSupervisorAnalytics(supervisorId, period = 'month') {
  ...
  const memberIdStrs = [
    ...new Set([
      String(supervisorId),
      ...groups.flatMap(g =>
        g.members.filter(m => m.status === 'active').map(m => String(m.userId))
      ),
    ]),
  ];
  ...
  const [forks, proposals, amendments] = await Promise.all([
    LawFork.find({ authorId: { $in: memberOids }, createdAt: { $gte: since } })
           .select('authorId status lawId createdAt').lean(),
    Proposal.find({ created_by: { $in: memberOids }, createdAt: { $gte: since } })
           .select('created_by status law_id createdAt').lean(),
    Amendment.find({ created_by: { $in: memberOids }, createdAt: { $gte: since } })
           .select('created_by createdAt').lean(),
  ]);
  ...
  const approvalRate      = totalPool > 0 ? Math.round((totalApproved / totalPool) * 100) : 0;
  ...
  forks.forEach(f => {
    ...
    if (f.status === 'approved') approvedByUser[id] = (approvedByUser[id] || 0) + 1;
  });
  proposals.forEach(p => {
    const id = String(p.created_by);
    proposPerUser[id] = (proposPerUser[id] || 0) + 1;
    if (p.status === 'approved') approvedByUser[id] = (approvedByUser[id] || 0) + 1;
  });
  ...
  let topLegislators = [];
  if (topIds.length) {
    const users = await User.find({ _id: { $in: topIds } }).select('fullName displayName').lean();
    const umap  = Object.fromEntries(users.map(u => [String(u._id), u.fullName || u.displayName || 'Unknown']));
  ...
  const lawForkMap  = {};
  const lawPropMap  = {};
  forks.forEach(f    => { if (f.lawId) lawForkMap[String(f.lawId)] = (lawForkMap[String(f.lawId)] || 0) + 1; });
  proposals.forEach(p => { if (p.law_id) lawPropMap[String(p.law_id)] = (lawPropMap[String(p.law_id)] || 0) + 1; });
  const topLawIds = Object.entries(lawForkMap).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([id]) => id);
  let lawCoverage = [];
  if (topLawIds.length) {
    const laws  = await Law.find({ _id: { $in: topLawIds } }).select('title').lean();
    const lmap  = Object.fromEntries(laws.map(l => [String(l._id), l.title || '—']));
...
export async function getLegislatorAnalytics(userId, period = 'month') {
  ...
  const [myForks, myProposals, myAmendments] = await Promise.all([
    LawFork.find({ authorId: oid, createdAt: { $gte: since } }).select('status createdAt').lean(),
    Proposal.find({ created_by: oid, createdAt: { $gte: since } }).select('status createdAt').lean(),
    Amendment.find({ created_by: oid, createdAt: { $gte: since } }).select('createdAt').lean(),
  ]);

  const approved = myForks.filter(f => f.status === 'approved').length
                 + myProposals.filter(p => p.status === 'approved').length;
  const rejected = myForks.filter(f => f.status === 'rejected').length
                 + myProposals.filter(p => p.status === 'rejected').length;
  const pending  = myForks.filter(f => f.status === 'review').length
  ...
  const approvalRate = approvalPool > 0 ? Math.round((approved / approvalPool) * 100) : 0;

  // Platform-wide averages for anonymous comparison
  const [allForks, allProposals, allAmendments] = await Promise.all([
    LawFork.find({ createdAt: { $gte: since } }).select('authorId').lean(),
    Proposal.find({ createdAt: { $gte: since } }).select('created_by').lean(),
    Amendment.find({ createdAt: { $gte: since } }).select('created_by').lean(),
  ]);

  const userCount = {};
  allForks.forEach(f      => { const id = String(f.authorId);   userCount[id] = (userCount[id] || 0) + 1; });
  allProposals.forEach(p  => { const id = String(p.created_by); userCount[id] = (userCount[id] || 0) + 1; });
  allAmendments.forEach(a => { const id = String(a.created_by); userCount[id] = (userCount[id] || 0) + 1; });

  const counts = Object.values(userCount);
  const platformAvg = counts.length ? Math.round(counts.reduce((a, b) => a + b, 0) / counts.length) : 0;

  // Group average
  const myGroups = await Group.find({ 'members.userId': oid, 'members.status': 'active' }).lean();
  const groupMemberIds = [...new Set(
    myGroups.flatMap(g => g.members.filter(m => m.status === 'active').map(m => String(m.userId)))
  )].filter(id => id !== String(userId));
  const groupCounts = groupMemberIds.map(id => userCount[id] || 0);
  const groupAvg = groupCounts.length ? Math.round(groupCounts.reduce((a, b) => a + b, 0) / groupCounts.length) : 0;
  ...
  return {
    ...
    weeklyActivity: buckets.map(b => ({
      week: b.label,
      actions: myForks.filter(f => inBucket(f.createdAt, b)).length

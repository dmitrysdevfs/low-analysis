...
export async function createGroup(supervisorId, { name, course, memberIds = [], trackedLawIds = [] }) {
  return SupervisorGroup.create({ supervisorId, name, course, memberIds, trackedLawIds });
}
...
export async function updateGroup(groupId, supervisorId, data) {
  ...
  if (!group) throw Object.assign(new Error('Group not found or not authorized'), { statusCode: 404 });
...
export async function getDashboardSummary(supervisorId) {
  ...
  const allMemberIds = [...new Set(groups.flatMap((g) => g.memberIds.map((m) => m._id.toString())))];
  const allLawIds = [...new Set(groups.flatMap((g) => g.trackedLawIds.map((l) => l._id.toString())))];
  ...
  for (const fork of forks) {
    ...
    if (!lawActivity[lawId]) {
      lawActivity[lawId] = { law: fork.law_id, forkCount: 0, proposalCount: 0, lastActivityAt: null };
    }
    lawActivity[lawId].forkCount += 1;
    const at = new Date(fork.updatedAt);
    if (!lawActivity[lawId].lastActivityAt || at > lawActivity[lawId].lastActivityAt) {
  ...
  for (const p of proposals) {
    ...
    if (!lawId) continue;
    if (!lawActivity[lawId]) {
      lawActivity[lawId] = { law: p.law_id, forkCount: 0, proposalCount: 0, lastActivityAt: null };
    }
  ...
  const recentActivity = [...forks.slice(0, 10).map((f) => ({
    type: 'fork',
    title: f.title,

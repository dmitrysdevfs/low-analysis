...
export const updateProposal = async (id, userId, updates) => {
  ...
  allowed.forEach((key) => {
    if (updates[key] !== undefined) proposal[key] = updates[key];
  });
...
export const submitProposal = async (id, userId) => {
  ...
  if (proposal.element_id) {
    ...
    if (conflict)
      throw Object.assign(
        new Error('Another active proposal exists for this element'),
        { status: 409 },
      );
  }
...
export const withdrawProposal = async (id, userId) => {
  ...
  if (!['draft', 'active'].includes(proposal.status))
    throw Object.assign(new Error('Cannot withdraw in current status'), {
      status: 400,
    });
...
export const getProposalsForElement = async (law_id, element_id) => {
  return await LawChangeProposal.find({ law_id, element_id }).sort({
    createdAt: -1,
  });
};

export const getMyProposals = async (userId) => {
  return await LawChangeProposal.find({ created_by: userId })
    .sort({ createdAt: -1 })

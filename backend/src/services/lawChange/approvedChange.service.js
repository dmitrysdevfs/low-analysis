...
export const getApprovedChangesForLaw = async (law_id) => {
  return await ApprovedChange.find({ law_id, is_current: true, status: 'active' })
    .sort({ approved_at: -1 })
...
export const getLawView = async (law_id) => {
  const [law, elements, approved_changes, active_proposals] = await Promise.all([
    ...
    ApprovedChange.find({ law_id, is_current: true, status: 'active' }).lean(),
...
export const rollbackChange = async (id, adminId) => {
  ...
  if (!change) throw Object.assign(new Error('ApprovedChange not found'), { status: 404 });
  ...
  if (change.supersedes_id) {
    await ApprovedChange.findByIdAndUpdate(change.supersedes_id, { is_current: true, status: 'active' });
  }

  // Also mark the winning proposal back to a different status
  await LawChangeProposal.findByIdAndUpdate(change.winning_proposal_id, { status: 'archived' });

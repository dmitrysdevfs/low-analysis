import RadiantProposal from '../models/RadiantProposal.js';
import RadiantProposalVote from '../models/RadiantProposalVote.js';
...
async function recalcVotes(proposal_id) {
  const votes = await RadiantProposalVote.find({ proposal_id });
  ...
  await RadiantProposal.findByIdAndUpdate(proposal_id, {
    votes_for_weighted: for_w,
    votes_against_weighted: against_w,
    votes_for_count: for_c,
    votes_against_count: against_c,
  });
  return await RadiantProposal.findById(proposal_id);
}

export const createProposal = async (data) => {
  return await RadiantProposal.create({ ...data, status: 'draft' });
};
...
export const getProposals = async (filters = {}, pagination = null) => {
  ...
  if (pagination) {
    ...
    const [proposals, total] = await Promise.all([
      RadiantProposal.find(query)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .populate('created_by', 'fullName role'),
      RadiantProposal.countDocuments(query),
    ]);
    return { proposals, total, page, pages: Math.ceil(total / limit) };
  }
  return await RadiantProposal.find(query)
    .sort({ createdAt: -1 })
    .populate('created_by', 'fullName role');
};

export const getMyProposals = async (userId) => {
  return await RadiantProposal.find({ created_by: userId }).sort({ createdAt: -1 });
};

export const getProposalById = async (id) => {
  const proposal = await RadiantProposal.findById(id).populate('created_by', 'fullName role');
  if (!proposal)
...
export const updateProposal = async (id, userId, patch) => {
  const proposal = await RadiantProposal.findById(id);
  if (!proposal)
    throw Object.assign(new Error('Proposal not found'), { status: 404 });
  ...
  const allowed = ['proposal_type', 'node_law_id', 'source_law_id', 'target_law_id', 'link_type', 'reason'];
  allowed.forEach((key) => {
...
export const castVote = async (proposalId, user, vote) => {
  ...
  const proposal = await RadiantProposal.findById(proposalId);
  if (!proposal)
    throw Object.assign(new Error('Proposal not found'), { status: 404 });
  ...
  await RadiantProposalVote.findOneAndUpdate(
  ...
  return await RadiantProposal.findById(proposalId);
};
...
export const removeVote = async (proposalId, userId) => {
  ...
  await RadiantProposalVote.findOneAndDelete({ proposal_id: proposalId, user_id: userId });
  return await recalcVotes(proposalId);
};

export const getVoteStats = async (proposalId, userId) => {
  const proposal = await RadiantProposal.findById(proposalId).lean();
  if (!proposal)
    throw Object.assign(new Error('Proposal not found'), { status: 404 });

  const myVote = userId
    ? await RadiantProposalVote.findOne({ proposal_id: proposalId, user_id: userId }).lean()
    : null;

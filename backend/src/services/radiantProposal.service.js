...
export const getMyProposals = async (userId) => {
  return await RadiantProposal.find({ created_by: userId }).sort({
    createdAt: -1,
  });
};

export const getProposalById = async (id) => {
  const proposal = await RadiantProposal.findById(id).populate(
    'created_by',
    'fullName role',
  );
  if (!proposal)
...
export const removeVote = async (proposalId, userId) => {
  ...
  await RadiantProposalVote.findOneAndDelete({
...
export const getVoteStats = async (proposalId, userId) => {
  ...
  const myVote = userId
    ? await RadiantProposalVote.findOne({
        proposal_id: proposalId,
        user_id: userId,
      }).lean()
    : null;

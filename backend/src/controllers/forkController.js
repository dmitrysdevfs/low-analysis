...
export const getFork = async (req, res, next) => {
  try {
    ...
    const fork = await forkService.getForkById(req.params.id, isOwner ? req.user._id : null);
...
export const addChange = async (req, res, next) => {
  try {
    const change = await forkService.addChange(req.params.id, req.user._id, req.body);
    res.json(change);
  } catch (err) { next(err); }
};

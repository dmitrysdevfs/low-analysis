...
export async function createFork(req, res) {
  try {
    const { name, description, viewState, filters, pathState, isPublic } =
      req.body;
    if (!name?.trim()) {
...
export async function getForkById(req, res) {
  try {
    ...
    if (
      !fork.isPublic &&
      fork.userId.toString() !== req.user?._id?.toString()
    ) {
...
export async function updateFork(req, res) {
  try {
    const fork = await GraphFork.findOne({
      _id: req.params.id,
      userId: req.user._id,
    });
    if (!fork)
      return res.status(404).json({ error: 'Fork not found or not yours' });
    const { name, description, viewState, filters, pathState, isPublic } =
      req.body;
...
export async function deleteFork(req, res) {
  try {
    const fork = await GraphFork.findOneAndDelete({
      _id: req.params.id,
      userId: req.user._id,
    });
    if (!fork)

...
describe("auth pages", () => {
  it("switches from login to register inside the same auth card", () => {
    ...
    expect(screen.queryByRole("button", { name: "Клієнт" })).not.toBeInTheDocument();
    expect(screen.getByLabelText("Логін або email")).toBeInTheDocument();
    expect(screen.getByLabelText("Пароль")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Увійти" })).toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "Перейти до вкладки реєстрації" }));
    ...
    expect(screen.getByRole("button", { name: "Створити адміна" })).toBeInTheDocument();
  });
  ...
  it("switches from register to login inside the same auth card", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Перейти до вкладки входу" }));
    ...
    expect(screen.queryByRole("button", { name: "Адмін" })).not.toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Увійти" })).toBeInTheDocument();
  });

...
describe('Home page', () => {
  it('renders the ukrainian hero and roadmap content', () => {
    ...
    expect(screen.getByText(/Перетворення текстів законів на структуру/i)).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /Переглянути закони/i })).toHaveAttribute(
      'href',
      '/laws'
    );
    expect(screen.getByRole('link', { name: /Суб'єкти/i })).toHaveAttribute('href', '/subjects');

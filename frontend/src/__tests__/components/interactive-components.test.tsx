...
describe('interactive frontend components', () => {
  it('highlights the active nav item in AppHeader', () => {
    ...
    expect(screen.getByRole('link', { name: 'Low Analysis' })).toHaveAttribute('href', '/');
  ...
  it('submits and resets search form parameters', async () => {
    ...
    await user.type(screen.getByPlaceholderText('Введіть ключові слова...'), 'конституція');
  ...
  it('renders loading, empty, error and results states in SearchResults', () => {
    const { rerender } = render(
      <SearchResults results={[]} loading={false} error={null} searched={false} query="" />
    );

    expect(screen.getByText(/Введіть запит/i)).toBeInTheDocument();

    rerender(<SearchResults results={[]} loading error={null} searched={false} query="" />);
    expect(screen.getByLabelText('Завантаження результатів пошуку')).toBeInTheDocument();
    ...
    rerender(
      <SearchResults results={[]} loading={false} error={null} searched query="конституція" />
    );
    ...
    expect(screen.getByText(/Знайдено: 2 закони/i)).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /КОНСТИТУЦІЯ УКРАЇНИ/i })).toHaveAttribute(
      'href',
      `/laws/${LAW_FIXTURE._id}`
    );
  ...
  it('renders dialog content and closes via the close button', async () => {
    ...
    render(
      <Dialog open onOpenChange={onOpenChange} title="Діалог" description="Опис">

...
describe('frontend API client', () => {
  ...
  it('throws a descriptive error on non-ok responses', async () => {
    ...
    await expect(getLaws('boom')).rejects.toThrow('HTTP 500: Internal Server Error');
  });
  ...
  it('returns the law tree response with law metadata and elements', async () => {
    ...
    await expect(getLawTree(LAW_FIXTURE._id)).resolves.toEqual(LAW_TREE_RESPONSE_FIXTURE);
  ...
  it('requests a single article by law id and article number', async () => {
    ...
    await expect(getArticle('law-1', '1')).resolves.toEqual(ARTICLE_RESPONSE_FIXTURE);
  ...
  it('requests subjects and subject elements', async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(2, `/api/subjects/${SUBJECT_FIXTURE._id}/elements`);
  });
  ...
  it('sends a POST request to parse a law by URL', async () => {
    ...
    await expect(parseLaw('https://zakon.rada.gov.ua/laws/show/254к/96-вр')).resolves.toEqual(
      response
    );
  ...
  it('uses backend error message for parseLaw when available', async () => {
    ...
    await expect(parseLaw('invalid-url')).rejects.toThrow('Could not extract valid law code');
  });
});

  ...
  useFrame(({ clock }) => {
    if (mat)
      mat.emissiveIntensity =
        0.38 + Math.sin(clock.getElapsedTime() * 2.4) * 0.18;
  });
...
function Book() {
  ...
  const fMat = useMemo(
    () =>
      new THREE.MeshStandardMaterial({
        ...
        roughness: 0.06,
        metalness: 1.0,
        emissive: new THREE.Color(0xc8a843),
  ...
  return (
    <group ref={groupRef} position={[0, 1.4, 0]}>
      ...
      <mesh
        material={fMat}
        position={[0, BH / 2 - mg - ft / 2, BD / 2 + fd / 2]}
      >
      ...
      <mesh
        material={fMat}
        position={[-(BW / 2 - mg - ft / 2), 0, BD / 2 + fd / 2]}
      >
...
function Pedestal() {
  ...
  useFrame(({ clock }) => {
    const t = clock.getElapsedTime();
    if (glowRef.current)
      glowRef.current.emissiveIntensity = 2.5 + Math.sin(t * 1.6) * 0.7;
    if (lightRef.current)
      lightRef.current.intensity = 5.0 + Math.sin(t * 1.6) * 1.2;
  });
...
function Particles() {
  ...
  useFrame(({ clock }) => {
    ...
    for (let i = 0; i < SN; i++) {
      ...
      sa[i * 3] = Math.cos(ang) * r;
      sa[i * 3 + 2] = Math.sin(ang) * r;
...
export default function BookCanvas() {
  return (
    <Canvas
      ...
      scene={{
        background: new THREE.Color(0x06101c),
        fog: new THREE.FogExp2(0x06101c, 0.055),
      }}
    >
      <Scene />
    </Canvas>

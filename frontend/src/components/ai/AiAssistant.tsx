  ...
  return (
    <div className={`${styles.container} hide-mobile`}>
      ...
      <button
        ...
        className={`${styles.circleTrigger} ${isOpen ? styles.triggerActive : ""}`}
        aria-label={isOpen ? "Закрити AI Аналітик" : "Відкрити AI Аналітик"}
      >
        {isOpen ? (
          <span className={styles.crossIcon}>&times;</span>
        ) : (
          <span className={styles.triggerText}>AI</span>
        )}
      </button>

      {/* Діалогове вікно, що плавно вилітає знизу */}
      <div
        className={`${styles.dialog} panel ${isOpen ? styles.dialogOpen : ""}`}
      >

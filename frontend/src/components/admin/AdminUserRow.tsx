...
function PortalMenu({ open, pos, width, onClose, triggerRef, children }: PortalMenuProps) {
...
export function AdminUserRow({ account, onAction }: AdminUserRowProps) {
  ...
  const [activePanel, setActivePanel] = useState<null | "info" | "actions">(null);
  ...
  const openInfo = useCallback(() => {
    ...
    setInfoPos({
      top: r.bottom + 6,
      left: Math.max(8, Math.min(r.left, (typeof window !== "undefined" ? window.innerWidth : 1200) - 292)),
    });
    setActivePanel("info");
  }, []);
  ...
  const toggleInfo    = useCallback(() => activePanel === "info"    ? setActivePanel(null) : openInfo(),    [activePanel, openInfo]);
  const toggleActions = useCallback(() => activePanel === "actions" ? setActivePanel(null) : openActions(), [activePanel, openActions]);
  const close = useCallback(() => setActivePanel(null), []);

  const handleAction = useCallback((action: AccountAction) => {
    setActivePanel(null);
    ...
    <>
      ...
      >
        <div className={styles.rowPopover}>
          ...
          <dl className={styles.rowPopoverGrid}>
            ...
            <dt>Статус</dt>
            <dd className={isInactive ? styles.rowPopoverDanger : styles.rowPopoverAccent}>
              {isInactive ? "Неактивний" : "Активний"}
            </dd>
            ...
            <dt>Вхід</dt>
            <dd>{account.lastLoginAt ? formatDateShort(account.lastLoginAt) : "ніколи"}</dd>
      ...
      >
        <div className={styles.rowActionsMenu}>
          ...
          >
            {account.accountType === "admin" ? "Знизити роль" : "Підвищити до адміна"}
          </button>

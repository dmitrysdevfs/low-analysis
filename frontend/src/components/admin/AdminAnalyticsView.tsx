...
export function AdminAnalyticsView() {
  ...
  const metrics = useMemo(() => {
    ...
    const totalParagraphs = laws.reduce((sum, law) => sum + (law.totalParagraphs ?? 0), 0);
    const averageArticles = laws.length > 0 ? (totalArticles / laws.length).toFixed(1) : "0";
    const statusDistribution = groupCounts(laws.map((law) => law.status ?? "Unknown"));
    const subjectStatusDistribution = groupCounts(subjects.map((subject) => subject.legal_status));
    ...
    <section className={styles.page}>
      <div className={styles.hero}>
        <div>
          ...
          <p className={styles.description}>
            Цей огляд агрегує відповіді `/api/laws` та `/api/subjects` для швидкого операційного огляду правового покриття та розподілу суб'єктів.
          </p>
        </div>

        <div className={styles.heroAside}>
          <span className={styles.tag}>Тільки фронтенд</span>
          <div className={styles.heroValue}>{laws.length + subjects.length} відстежуваних об'єктів</div>
          <div className={styles.heroMeta}>
            Закони, суб'єкти, глибина статей та метадані обчислюються на фронтенді з наявних API-запитів.
          </div>
      ...
      <div className={styles.metricsGrid}>
        <article className={styles.metricCard}>
          ...
          <p className={styles.metricNote}>Всього правових документів з `/api/laws`.</p>
        </article>

        <article className={styles.metricCard}>
          <span className={styles.metricLabel}>Суб'єкти</span>
          <strong className={styles.metricValue}>{subjects.length}</strong>
          <p className={styles.metricNote}>Визначених правових суб'єктів з `/api/subjects`.</p>
        </article>
        ...
        <article className={styles.metricCard}>
          ...
          <p className={styles.metricNote}>Глибина абзаців в метаданих законів.</p>
        </article>

        <article className={styles.metricCard}>
          <span className={styles.metricLabel}>Сер. статей</span>
          <strong className={styles.metricValue}>{metrics.averageArticles}</strong>
          <p className={styles.metricNote}>Середня кількість статей на закон.</p>
        </article>
      </div>
      ...
      <div className={styles.contentGrid}>
            ...
            <div className={styles.distributionList}>
              {metrics.subjectStatusDistribution.map(([label, count]) => (
                <div key={label} className={styles.distributionRow}>
                  ...
                  <div className={styles.distributionMeta}>
                    {subjects.length > 0 ? Math.round((count / subjects.length) * 100) : 0}% суб'єктів
                  </div>
                </div>
              ))}
            </div>
        ...
        <article className={`${styles.panel} ${styles.panelWide}`}>
          ...
          {metrics.recentLaws.length > 0 ? (
                  ...
                  <div className={styles.listTopRow}>
                    <div>
                      ...
                      <div className={styles.listMeta}>
                        Created {formatDateMedium(law.createdAt)} · {law.totalArticles} articles ·{" "}
            ...
            <div className={styles.emptyState}>Законів у поточній відповіді немає.</div>
          )}
        </article>
        ...
        <article className={styles.panel}>
          ...
          <div className={styles.list}>
            ...
            <div className={styles.listRow}>
              ...
              <div className={styles.listMeta}>
                {subjectsLoading ? "Завантаження…" : subjectsError ? subjectsError : "Готово"}
              </div>
            </div>
          </div>
        </article>

...
import type { ApiCenterPayload, ApiEndpointRecord, ApiSecurityMode } from "../types";
...
export function ApiCenterView({ payload }: { payload: ApiCenterPayload }) {
  const model = useMemo(() => createApiCenterModel(payload.spec), [payload.spec]);
    ...
    <section className={styles.page}>
      ...
      <section className={styles.metrics}>
        <article className={styles.metricCard}>
          ...
          <div className={styles.metricMeta}>Усі маршрути, що потрапили в OpenAPI.</div>
        </article>
        <article className={styles.metricCard}>
          <div className={styles.metricLabel}>
            <BookOpen size={14} />
          ...
          <div className={styles.moduleList}>
            ...
            >
              <div className={styles.moduleTop}>
                ...
                <span className={styles.moduleCount}>{model.endpoints.length}</span>
              </div>
        ...
        <div className={styles.panel}>
          <div className={styles.panelHeader}>
            <div>
              ...
              <p className={styles.panelSubtitle}>
                Пошук по path, summary та tags. Фільтри не змінюють backend, лише
                допомагають швидко читати специфікацію.
              </p>
            </div>
              ...
              filteredEndpoints.map((endpoint) => (
                ...
                >
                  ...
                  <div className={styles.chipRow}>
                    ...
                    <span className={styles.chip}>Params: {endpoint.parameterCount}</span>
                    <span className={styles.chip}>
                      Responses: {endpoint.responseCodes.join(", ")}
                    </span>
                    {endpoint.hasRequestBody && (
                      <span className={styles.chip}>
                        Body {endpoint.requestBodyRequired ? "required" : "optional"}
                      </span>
                    )}
                  </div>
                </button>
        ...
        <aside className={styles.panel}>
          <div className={styles.panelHeader}>
            <div>
              ...
              <p className={styles.panelSubtitle}>
                Summary, auth, params, body та response-коди для вибраного маршруту.
              </p>
            </div>
          ...
          {!selectedEndpoint ? (
            ...
            <div className={styles.detailBody}>
              <div className={styles.detailHeadline}>
                ...
                <p className={styles.endpointSummary}>{selectedEndpoint.summary}</p>
                <p className={styles.detailText}>{selectedEndpoint.description}</p>
              </div>
              ...
              <div className={styles.chipRow}>
                ...
                {selectedEndpoint.deprecated && <span className={styles.chip}>Deprecated</span>}
              </div>
              ...
              <div className={styles.detailGrid}>
                <article className={styles.detailCard}>
                  ...
                  {selectedEndpoint.operation.parameters?.length ? (
                    <ul className={styles.list}>
                      {selectedEndpoint.operation.parameters.map((parameter) => (
                ...
                <article className={styles.detailCard}>
                  ...
                  {selectedEndpoint.operation.requestBody ? (
                    ...
                    <p className={styles.detailText}>Body для цього endpoint-а не описаний.</p>
                  )}
                </article>
                ...
                <article className={styles.detailCard}>
                  ...
                  {selectedEndpoint.responseCodes.length ? (
                    ...
                    <p className={styles.detailText}>Response-коди не вказані.</p>
                  )}
                </article>

                <article className={styles.detailCard}>
                  <span className={styles.detailLabel}>Links</span>
                  <p className={styles.detailMono}>Swagger UI: {payload.swaggerUrl}</p>
                  <p className={styles.detailMono}>OpenAPI JSON: {payload.openApiUrl}</p>
                </article>
              </div>
            </div>
          )}

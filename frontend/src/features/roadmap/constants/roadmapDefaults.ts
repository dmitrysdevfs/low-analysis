import type { RoadmapContent } from "../types";

export const ROADMAP_DEFAULTS: RoadmapContent = {
  phases: [
    {
      id: "phase-1",
      label: "Фаза 1 — Парсинг та структура",
      status: "done",
      tasks: [
        { text: "Cheerio-парсинг законів з zakon.rada.gov.ua", done: true },
        { text: "MongoDB Atlas: зберігання дерева елементів", done: true },
        { text: "REST API для закону, дерева та статей", done: true },
        { text: "Ієрархічний код rз→ст→ч для кожного елемента", done: true },
      ],
    },
    {
      id: "phase-2",
      label: "Фаза 2 — NLP та аналітика",
      status: "done",
      tasks: [
        { text: "Виявлення суб'єктів регулювання (NLP)", done: true },
        {
          text: "Z-score та рівні ризику: зелений / жовтий / червоний",
          done: true,
        },
        { text: "Таксономія норм: домени, функції, ключові слова", done: true },
        { text: "Теплова карта розподілу норм по закону", done: true },
      ],
    },
    {
      id: "phase-3",
      label: "Фаза 3 — UI та особистий кабінет",
      status: "done",
      tasks: [
        {
          text: "Ієрархічний перегляд закону: розділ → стаття → абзац",
          done: true,
        },
        { text: "Пошук і фільтрація по назві, статусу та даті", done: true },
        { text: "Нотатки з drag-and-drop зі статей", done: true },
        {
          text: "Особистий кабінет: збережені матеріали та нотатки",
          done: true,
        },
      ],
    },
    {
      id: "phase-4",
      label: "Фаза 4 — AI та адмін-панель",
      status: "done",
      tasks: [
        { text: "AI-асистент для запитів до норм закону", done: true },
        { text: "Адмін-панель: аналітика, користувачі, аудит", done: true },
        { text: "Білінг-плани, квоти та призначення", done: true },
        { text: "Супер-код, матриця доступу, ролева модель", done: true },
      ],
    },
    {
      id: "phase-5",
      label: "Фаза 5 — Масштабування",
      status: "done",
      tasks: [
        {
          text: "Кодекси України: КУпАП, Цивільний, Кримінальний",
          done: true,
        },
        { text: "Граф зв'язків між нормами різних законів", done: true },
        { text: '"Радіант" — 3D-візуалізація законодавчої бази', done: true },
        { text: "Повнотекстовий пошук на рівні бекенду", done: true },
      ],
    },
    {
      id: "phase-6",
      label: "Фаза 6 — Collaborative Lawmaking",
      status: "in_progress",
      description: "Створення середовища спільної роботи над законодавством.",
      businessValue:
        "Перехід від читання законів до створення законодавчих змін.",
      tasks: [
        { text: "Fork законів", done: true },
        { text: "Diff Viewer", done: true },
        { text: "Proposal System", done: true },
        { text: "Version History", done: false },
        { text: "Review Workflow", done: false },
        { text: "Approval Workflow", done: false },
        { text: "Порівняння редакцій", done: false },
        { text: "Колективна робота над змінами", done: false },
      ],
    },
    {
      id: "phase-7",
      label: "Фаза 7 — Educational Platform",
      status: "current_focus",
      description:
        "Платформа стає навчальним середовищем для студентів та інтернів.",
      businessValue: "Практичне навчання роботі із законодавством.",
      tasks: [
        { text: "Supervisor Role", done: true },
        { text: "Supervisor Dashboard", done: true },
        { text: "Навчальні групи", done: true },
        { text: "Моніторинг активності студентів", done: false },
        { text: "Оцінювання роботи", done: false },
        { text: "Рейтинг учасників", done: false },
        { text: "Практичні завдання", done: false },
        { text: "Навчальні сценарії", done: false },
      ],
    },
    {
      id: "phase-8",
      label: "Фаза 8 — Public Accountability",
      status: "planned",
      description: "Інструменти громадського контролю якості законодавства.",
      businessValue: "Підвищення прозорості законотворчого процесу.",
      tasks: [
        { text: "Рейтинг законопроєктів", done: false },
        { text: "Рейтинг депутатів", done: false },
        { text: "Рейтинг авторів змін", done: false },
        { text: "Аналітика активності", done: false },
        { text: "Історія голосувань", done: false },
        { text: "Профіль законотворця", done: false },
        { text: "Публічний трекінг змін", done: false },
      ],
    },
    {
      id: "phase-9",
      label: "Фаза 9 — AI Legal Intelligence",
      status: "planned",
      description: "Автоматизований аналіз законодавства за допомогою AI.",
      businessValue: "Автоматизація юридичного аналізу.",
      tasks: [
        { text: "AI Review законопроєкту", done: false },
        { text: "AI пошук конфліктів норм", done: false },
        { text: "AI пошук дублювання", done: false },
        { text: "AI оцінка ризиків", done: false },
        { text: "AI оцінка впливу", done: false },
        { text: "AI рекомендації", done: false },
        { text: "AI аналіз колізій", done: false },
        { text: "AI аналіз відповідності законодавству ЄС", done: false },
      ],
    },
    {
      id: "phase-10",
      label: "Фаза 10 — Legal Research Platform",
      status: "research",
      description:
        "Наукова та дослідницька інфраструктура для юридичних досліджень.",
      businessValue: "Law Analysis як база для юридичних досліджень.",
      tasks: [
        { text: "Наукові дослідження", done: false },
        { text: "Юридичні лабораторії", done: false },
        { text: "Публікації", done: false },
        { text: "Правові кейси", done: false },
        { text: "Дослідницькі групи", done: false },
        { text: "Університетські кабінети", done: false },
        { text: "Освітні програми", done: false },
      ],
    },
    {
      id: "phase-11",
      label: "Фаза 11 — Government Integration",
      status: "vision",
      description: "Інтеграція з державними системами та реєстрами.",
      businessValue: "Єдине джерело законодавчих даних.",
      tasks: [
        { text: "Інтеграція з Верховною Радою", done: false },
        { text: "Автоматичне оновлення законів", done: false },
        { text: "OpenData Integration", done: false },
        { text: "Єдиний API законодавства", done: false },
        { text: "Інтеграція з державними реєстрами", done: false },
        { text: "Моніторинг законодавчих змін", done: false },
      ],
    },
    {
      id: "phase-12",
      label: "Фаза 12 — International Legal Platform",
      status: "vision",
      description:
        "Розширення на міжнародне право та порівняльне законодавство.",
      businessValue: "Міжнародна платформа аналізу права.",
      tasks: [
        { text: "Законодавство ЄС", done: false },
        { text: "Порівняльне право", done: false },
        { text: "Мультимовність", done: false },
        { text: "International Analytics", done: false },
        { text: "Cross-country Comparison", done: false },
        { text: "EU AI Act Analysis", done: false },
        { text: "Legal Harmonization Tools", done: false },
      ],
    },
  ],
  roadmapItems: [
    {
      text: "Структурований перегляд законів: розділ → стаття → абзац",
      done: true,
    },
    { text: "Пошук і фільтрація по назві, статусу та даті", done: true },
    {
      text: "Суб'єкти регулювання з ролями та інтерактивною фільтрацією",
      done: true,
    },
    {
      text: "Статистика елементів: Z-score, рівні ризику, теплова карта",
      done: true,
    },
    {
      text: "Таксономія норм: правові функції, домени, ключові слова",
      done: true,
    },
    {
      text: "Нотатки з drag-and-drop зі статей та особистий кабінет",
      done: true,
    },
    { text: "AI-асистент для запитів до будь-якої норми закону", done: true },
    {
      text: "Адмін-панель: аналітика, керування користувачами, аудит",
      done: true,
    },
  ],
  deferredItems: [
    {
      title: "Повнотекстовий пошук на бекенді",
      reason:
        "Для поточних 11 законів клієнтська фільтрація достатня. Потрібен при виході на 100+ документів.",
    },
    {
      title: "WebSocket real-time сповіщення",
      reason:
        "Платформа read-only, real-time не критично. Повернемось при появі collaborative-режиму.",
    },
    {
      title: "PDF-парсинг документів",
      reason:
        "Складна структура таблиць і форматування. Потребує окремого парсера та ручної верифікації.",
    },
    {
      title: "Мобільний застосунок",
      reason:
        "Поза MVP-скоупом. Поточний responsive-UI покриває мобільні браузери достатньо.",
    },
  ],
  decisions: [
    {
      id: "d-1",
      title: "MongoDB Atlas Free Tier",
      decision: "Використовуємо хмарну MongoDB замість локальної бази",
      rationale:
        "Нульова операційна вартість на старті. Free Tier достатній для поточних обсягів. Вбудований Atlas Search при масштабуванні.",
    },
    {
      id: "d-2",
      title: "Next.js App Router",
      decision:
        "SSR для публічних сторінок, Client Components для інтерактивності",
      rationale:
        "SEO для сторінок законів. Server Components скорочують JS-бандл. Routing-first архітектура спрощує навігацію.",
    },
    {
      id: "d-3",
      title: "LOCAL_MODE = true в адмін-панелі",
      decision: "Адмін-дані з localStorage до готовності повного backend API",
      rationale:
        "Дозволяє розробляти і демонструвати UI незалежно від бекенду. Перемикач в одному файлі — легко прибрати.",
    },
    {
      id: "d-4",
      title: "TanStack Query v5",
      decision: "Server-state management замість Redux для всіх API-запитів",
      rationale:
        "Автоматичний кеш, deduplicated запити, devtools. Менше boilerplate ніж Redux Toolkit для read-heavy платформи.",
    },
    {
      id: "d-5",
      title: "Cheerio замість Puppeteer",
      decision: "HTML-парсинг через Cheerio (легкий) а не headless browser",
      rationale:
        "zakon.rada.gov.ua повертає статичний HTML. Puppeteer зайвий overhead. Cheerio 10x швидший для batch-парсингу.",
    },
    {
      id: "d-6",
      title: "Framer Motion для анімацій",
      decision:
        "Декларативні анімації через Framer Motion замість CSS-тільки підходу",
      rationale:
        "AnimatePresence для mount/unmount. Stagger-ефекти для списків. whileInView для scroll-triggered анімацій без IntersectionObserver вручну.",
    },
    {
      id: "d-7",
      title: "LLM як семантичний шар (SRL)",
      decision:
        "Використання LLM (Gemini API) для семантичного аналізу (Semantic Role Labeling) та витягування суб'єктів регулювання",
      rationale:
        "Українське законодавство має складну синтаксичну структуру. Класичні NLP-бібліотеки або регулярні вирази дають низьку точність на непрямих формулюваннях норм. Двошарова архітектура (Cheerio для синтаксичного дерева + LLM для семантичного виділення акторів) дозволяє виявляти суб'єктів з точністю >95% та гнучко будувати зв'язки.",
    },
  ],
  whatWeBuilding: [
    "аналізу законодавства",
    "навчання студентів",
    "роботи законотворців",
    "громадського контролю",
    "юридичних досліджень",
    "AI-підтримки правотворчості",
  ],
  visionStatement:
    "Стати відкритою екосистемою, де громадяни, студенти, юристи, науковці та законотворці можуть спільно аналізувати, покращувати та створювати законодавство.",
};

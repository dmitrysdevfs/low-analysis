...
const SIDEBAR_NAV = [
  { icon: <Eye size={20} />, label: "Нагляд", href: ROUTES.legislatorCabinet, active: true },
...
function LegislatorSidebar({ initials, name, role }: { initials: string; name: string; role: string }) {
  return (
...
export default function LegislatorCabinetPage() {
  ...
  const roleLabel = isAdmin ? "Адміністратор" : isSupervisor ? "Супервайзер" : "Законотворець";
  ...
  return (
    <div className={styles.workspace}>
      ...
      />
      <main className={styles.mainScroll}>
        <LegislatorDashboardView isSupervisor={isSupervisor && !isLegislator && !isAdmin} />
      </main>
    </div>
  );
}
...
function LegislatorDashboardView({ isSupervisor }: { isSupervisor: boolean }) {
  ...
  const activityFeed = useMemo(() => {
    const items: { label: string; time: string; type: "fork" | "graph" | "proposal" }[] = [
      ...forks.map((f) => ({
  ...
  const handleCreateFork = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!forkTitle.trim() || !forkLawId) { notify.warning("Вкажіть назву та закон"); return; }
    try {
    ...
    } catch { notify.error("Не вдалося створити форк"); }
  };
  ...
  const handleAddChange = async (e: React.FormEvent) => {
    ...
    } catch { notify.error("Не вдалося додати зміну"); }
  };
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={`${styles.hero} panel`}>
        <div className={styles.heroCopy}>
          ...
          <p className={styles.description}>
            Контролюйте процес підготовки та подання від ідеї до розгляду. Відстежуйте
            статус змін та отримуйте сигнали про потрібні дії.
          </p>
          {!isSupervisor && (
            <div className={styles.heroActions}>
              <button type="button" className="btn btn-primary" onClick={() => setShowForkForm((v) => !v)}>
                <Plus size={14} />
        ...
        <aside className={styles.heroAside}>
          ...
          <div className={styles.asideStats}>
            {[
              { icon: <FileText size={22} />, value: proposals.length, label: "Пропозиції" },
              { icon: <GitBranch size={22} />, value: draftForks.length, label: "Чернетки" },
              { icon: <Activity size={22} />, value: submittedForks.length, label: "На розгляді" },
              { icon: <Network size={22} />, value: graphForks.length, label: "Форки графу" },
            ].map((stat) => (
        ...
        <section className={`${styles.formPanel} panel`}>
          <div className={styles.sectionHeader}>
            <div>
              ...
              <h2 className={styles.sectionTitle}>Створення форку законопроекту</h2>
            </div>
          </div>
          <form className={styles.formGrid} onSubmit={handleCreateFork}>
            ...
            <div className={styles.formActions}>
              <button type="button" className="btn btn-outline" onClick={() => setShowForkForm(false)}>
                Скасувати
              </button>
              <button type="submit" className="btn btn-primary" disabled={createForkMutation.isPending}>
                {createForkMutation.isPending ? "Збереження..." : "Створити форк"}
              </button>
            </div>
          </form>
        </section>
      ...
      <div className={styles.kpiGrid}>
        {[
          { label: "Пропозиції", value: proposals.length, note: "Активних пропозицій до законів", icon: <FileText size={20} />, tone: "blue" },
          { label: "Форки законів", value: forks.length, note: "Форків законопроектів у всіх статусах", icon: <GitBranch size={20} />, tone: "gold" },
          { label: "Поправки", value: 0, note: "Поправки до чинних законів", icon: <PenLine size={20} />, tone: "teal" },
          { label: "Форки графу", value: graphForks.length, note: "Збережені стани графу зв&apos;язків", icon: <Network size={20} />, tone: "violet" },
        ].map((card) => (
          <article key={card.label} className={`${styles.kpiCard} panel`}>
            ...
            <p className={styles.kpiNote} dangerouslySetInnerHTML={{ __html: card.note }} />
          </article>
        ))}
      </div>
      ...
      <div className={styles.twoCol}>
        <section className={`${styles.sectionPanel} panel`}>
          <div className={styles.sectionHeader}>
            ...
            {!isSupervisor && (
              <button type="button" className={styles.sectionLink} onClick={() => setShowForkForm(true)}>
          ...
          {forksLoading ? (
            ...
            <div className={styles.emptyState}>
              ...
              <span className={styles.emptyDesc}>
                Створіть перший форк, щоб почати роботу над змінами до законопроекту.
              </span>
              {!isSupervisor && (
                <button type="button" className={styles.emptyBtn} onClick={() => setShowForkForm(true)}>
                  ...
                  <div key={fork._id} className={styles.forkCard}>
                    <div className={styles.forkHead}>
                      <div>
                        ...
                        <h3 className={styles.forkTitle}>{fork.title ?? "Без назви"}</h3>
                      </div>
                      <span className={`${styles.statusBadge} ${styles[`statusBadge_${fork.status === "draft" ? "draft" : fork.status === "approved" ? "good" : fork.status === "rejected" ? "danger" : "warning"}`]}`}>
                        {fork.status}
                      </span>
                    </div>
                    {fork.updatedAt && (
                    ...
                    <div className={styles.forkActions}>
                      <button type="button" className={styles.inlineLink} onClick={() => setDiffForkId(fork._id)}>
                      ...
                      {fork.status === "draft" && !isSupervisor && (
                        <>
                          <button
                            ...
                            onClick={() => setAddChangeForkId(isEditing ? null : fork._id)}
                          >
                            <PenLine size={14} /> {isEditing ? "Закрити" : "+ Зміна"}
                          </button>
                          <button
                            type="button"
                      ...
                      <form onSubmit={handleAddChange} className={styles.changeForm}>
                        ...
                        {changeOp !== "add" && (
                          <label className={styles.field}>
                            ...
                            <textarea className="form-control" value={changeOriginal} onChange={(e) => setChangeOriginal(e.target.value)} rows={2} />
                          </label>
                        )}
                        {changeOp !== "delete" && (
                          <label className={styles.field}>
                            <span>Пропонований текст</span>
                            <textarea className="form-control" value={changeProposed} onChange={(e) => setChangeProposed(e.target.value)} rows={2} />
                          </label>
                        )}
                        <label className={styles.field}>
                          <span>Обґрунтування</span>
                          <textarea className="form-control" value={changeRationale} onChange={(e) => setChangeRationale(e.target.value)} rows={2} />
                        </label>
                        <div className={styles.formActions}>
                          <button type="button" className="btn btn-outline" onClick={() => setAddChangeForkId(null)}>Скасувати</button>
                          <button type="submit" className="btn btn-primary" disabled={addChangeMutation.isPending}>
                            {addChangeMutation.isPending ? "Збереження..." : "Зберегти зміну"}
                          </button>
                        </div>
                      </form>
                    )}
        ...
        <section className={`${styles.sectionPanel} panel`}>
          ...
          <div className={styles.priorityList}>
            {[
              { color: "orange", title: "Чернетки форків", desc: "Форки зі статусом draft очікують на доопрацювання або подання" },
              { color: "purple", title: "На розгляді", desc: "Подані форки та пропозиції чекають рішення модератора" },
              { color: "teal", title: "Погоджені зміни", desc: "Зміни, що вже отримали схвалення і вступили в дію" },
              { color: "blue", title: "Форки графу", desc: "Збережені стани графу зв&apos;язків між нормами" },
            ].map((item) => (
      ...
      <section className={`${styles.sectionPanel} panel`}>
        <div className={styles.sectionHeader}>
          <div>
            ...
            <h2 className={styles.sectionTitle}>Пропозиції до законопроектів</h2>
          </div>
        </div>
        {proposals.length === 0 ? (
          <div className={styles.emptyState}>
            ...
            <span className={styles.emptyDesc}>
              Коли ви створите пропозиції до законопроектів, вони з&apos;являться тут.
            </span>
          </div>
        ) : (
          <MyProposalsList />
                ...
                <article key={gf._id} className={styles.feedItem}>
                  ...
                  <div className={styles.feedBody}>
                    <div className={styles.feedTop}>
                      ...
                      <span className={styles.feedDate}>{formatRelativeDate(gf.updatedAt)}</span>
                    </div>
            ...
            <div className={styles.feedList}>
              {activityFeed.map((item, i) => (
                <article key={i} className={styles.feedItem}>
                  <span className={`${styles.feedIcon} ${item.type === "graph" ? styles.feedIconFork : styles.feedIconProposal}`}>
                    {item.type === "graph" ? <Network size={14} /> : <GitBranch size={14} />}
                  </span>
                  <div className={styles.feedBody}>
                    <div className={styles.feedTop}>
                      <strong>{item.label}</strong>
                      <span className={styles.feedDate}>{formatRelativeDate(item.time)}</span>
                    </div>
                    <div className={styles.feedBottom}>
                      <span className={styles.feedStatus}>{item.type === "graph" ? "граф" : "форк"}</span>
                    </div>
                  </div>
                </article>
              ))}

...
const TYPE_LABELS: Record<GraphProposalType, string> = {
  add_law: "Додати закон",
  remove_law: "Видалити закон",
  add_edge: "Додати зв'язок",
  remove_edge: "Видалити зв'язок",
};
...
// ── Vote buttons for a single proposal ───────────────────────────────────────

function ProposalVoteRow({ proposal }: { proposal: GraphProposal }) {
  const { data: voteStats } = useGraphVoteStats(
    proposal.status === "active" ? proposal._id : null,
  );
  const castVote = useCastGraphVote();
  const removeVote = useRemoveGraphVote();
  ...
  async function handleVote(direction: "for" | "against") {
    try {
      ...
      if (msg.toLowerCase().includes("автор")) {
...
function ProposalCard({ proposal }: { proposal: GraphProposal }) {
...
function CreateProposalForm({ onClose }: { onClose: () => void }) {
  ...
  const isEdge = proposalType === "add_edge" || proposalType === "remove_edge";
  ...
  function buildPayload() {
    if (isEdge) {
      return {
        ...
        edge_type: edgeType.trim() || undefined,
        reason: reason.trim(),
      };
    }
    return {
  ...
  return (
    <div className={styles.formPanel}>
      <div className={styles.formHeader}>
        <h2 className={styles.formTitle}>Нова пропозиція до Графу</h2>
      ...
      <div className={styles.formBody}>
        <label className={styles.field}>
          ...
          >
            ...
            <option value="add_edge">Додати зв'язок</option>
            <option value="remove_edge">Видалити зв'язок</option>
          </select>
        </label>
...
export default function GraphProposalsPage() {
  ...
  const { data, isLoading, error } = useGraphProposals({
    ...
    <Layout>
      <div className={styles.page}>
        <div className={styles.header}>
          <div>
            <h1 className={styles.title}>Пропозиції до Графу</h1>
            <p className={styles.subtitle}>
              Голосування щодо змін у графі зв'язків між законами
            </p>
          </div>
          {canCreate && (
            <button

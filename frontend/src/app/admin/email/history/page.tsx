...
export default function AdminEmailHistoryPage() {
  ...
  const totalSent = campaigns?.filter((c) => c.status === "sent").reduce((s, c) => s + c.deliveredCount, 0) ?? 0;
  ...
  const avgOpen = (() => {
    ...
    return Math.round(withOpen.reduce((s, c) => s + (c.openCount / (c.deliveredCount || 1)), 0) / withOpen.length * 100);
  })();
    ...
    <div className={styles.root}>
      ...
      <div className={styles.kpiStrip}>
        <div className={styles.kpiItem}>
          <Mail size={14} className={styles.kpiIcon} style={{ color: "#4a80d4" }} />
          <div>
        ...
        <div className={styles.kpiItem}>
          <Users size={14} className={styles.kpiIcon} style={{ color: "#c8a843" }} />
          <div>
            <div className={styles.kpiNum}>{totalRecipients.toLocaleString("uk-UA")}</div>
            <div className={styles.kpiLabel}>Отримувачів</div>
          </div>
        </div>
        <div className={styles.kpiItem}>
          <MailCheck size={14} className={styles.kpiIcon} style={{ color: "#52b788" }} />
          <div>
            <div className={styles.kpiNum}>{totalSent.toLocaleString("uk-UA")}</div>
            <div className={styles.kpiLabel}>Доставлено</div>
          </div>
        </div>
        <div className={styles.kpiItem}>
          <TrendingUp size={14} className={styles.kpiIcon} style={{ color: "#a78bfa" }} />
          <div>
      ...
      <div className={styles.toolbar}>
        ...
        <div className={styles.toolbarRight}>
          <button type="button" className={styles.toolbarBtn} onClick={() => refetch()}>
            <RefreshCw size={13} />
          </button>
      ...
      {/* Main content */}
      <div className={`${styles.content} ${selected ? styles.contentWithInspector : ""}`}>
        {/* List */}
          ...
          {filtered.map((c) => (
            ...
            >
              <div className={styles.rowLeft}>
                <div className={`${styles.statusDot} ${styles[`dot_${c.status}`]}`} />
              </div>
              <div className={styles.rowMain}>
                <div className={styles.subject}>{c.subject}</div>
                <div className={styles.meta}>
                  {c.templateSlug} · {c.theme} · {formatDate(c.sentAt ?? c.createdAt)}
                </div>
              </div>
              ...
              <ChevronRight size={14} className={`${styles.rowArrow} ${selected?._id === c._id ? styles.rowArrowOpen : ""}`} />
            </div>
          ))}
        </div>
        ...
        {selected && (
          <div className={styles.inspector}>
            <div className={styles.inspectorHeader}>
              ...
              <button type="button" className={styles.inspectorClose} onClick={() => setSelected(null)}>
                <X size={14} />
              </button>
            </div>
            ...
            <div className={styles.inspectorRow}>
              <div className={styles.inspectorSection}>
                ...
                <div className={styles.inspectorValue}>{selected.templateSlug}</div>
              </div>
              <div className={styles.inspectorSection}>
            ...
            <div className={styles.inspectorStats}>
              <div className={styles.inspectorStat}>
                ...
                <div>
                  <div className={styles.inspectorStatNum}>{selected.recipientCount}</div>
              ...
              <div className={styles.inspectorStat}>
                ...
                <div>
                  <div className={styles.inspectorStatNum}>{selected.deliveredCount}</div>
              ...
              <div className={styles.inspectorStat}>
                ...
                <div>
                  <div className={styles.inspectorStatNum}>{selected.openCount || "—"}</div>
              ...
              <div className={styles.inspectorStat}>
                ...
                <div>
                  <div className={styles.inspectorStatNum}>{formatDate(selected.sentAt)}</div>
                  <div className={styles.inspectorStatLabel}>Надіслано</div>
                </div>
              </div>
            </div>

            <div className={styles.inspectorLogsHeader}>Логи ({logs?.length ?? 0})</div>
            <div className={styles.inspectorLogs}>
              ...
              {logs?.slice(0, 12).map((log, i) => (
                <div key={i} className={styles.logRow}>
                  <div className={`${styles.logDot} ${log.status === "delivered" ? styles.logDotOk : styles.logDotErr}`} />
